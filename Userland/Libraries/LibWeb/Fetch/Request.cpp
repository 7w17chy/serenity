...
WebIDL::ExceptionOr<JS::NonnullGCPtr<Request>> Request::construct_impl(JS::Realm& realm, RequestInfo const& input, RequestInit const& init)
{
    ...
    if (input.has<String>()) {
        ...
        auto parsed_url = TRY_OR_THROW_OOM(vm, URLParser::parse(input.get<String>(), &base_url));
    ...
    if (init.referrer.has_value()) {
        ...
        // 3. Otherwise:
            ...
            auto parsed_referrer = TRY_OR_THROW_OOM(vm, URLParser::parse(referrer, &base_url));
